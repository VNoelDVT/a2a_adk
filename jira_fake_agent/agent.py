...
# journal des jobs par transactionId
JOBS: Dict[str, Dict[str, Any]] = {}  # txid -> {action,status,error,result,ts,payload}

# --- normalisation projet: accepte Phoenix-V2 ET Phoenix_V2 ---
...
def _tickets_for_project(project: str):
    cp = _canon(project)
...
def findRisks(project: str) -> dict:
    ...
    - high: priority Highest & status != Done
    - blockers: liens is_blocked_by
    - controversial: comments_count >= 10
    """
...
def createTicket(
    ...
    """Crée un ticket et journalise l'opération; simulateFailure=True simule un 403."""
    txid = str(uuid.uuid4())
    now = time.time()
    payload = {
        "project": project, "summary": summary, "description": description,
    ...
    if simulateFailure:
        JOBS[txid] = {
            "action": "createTicket", "status": "Failed",
            "error": "Jira API returned 403 Forbidden (simulé). Vérifiez les permissions.",
            "result": None, "ts": now, "payload": payload
        }
        return {"status": "failed", "transactionId": txid, "error": JOBS[txid]["error"]}
...
root_agent = Agent(
    name="jira_fake_agent",
    model="gemini-2.0-flash-001",
    description="Agent Jira factice (findRisks, createTicket, getTicketLogs) pour démo A2A.",
    instruction=(
        "Réponds en français. Utilise findRisks(project), createTicket(...), getTicketLogs(transactionId)."
    ),
    tools=[findRisks, createTicket, getTicketLogs],
